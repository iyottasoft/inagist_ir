...
inagist::GistCollector g_gist_collector;
...
extern "C"
#endif
int InitGistCollector(const char* keytuples_extracter_config_file,
                  const char* language_detection_config_file,
                  const char* text_classification_config_file) {

  if (g_gist_collector.Init(keytuples_extracter_config_file
#ifdef LANG_ENABLED
...
int CallGetGist(unsigned char* text_buffer, const unsigned int text_buffer_len,
  ...
  int ret_value = 0;
  if ((ret_value = g_gist_collector.GetGist(text_buffer, text_buffer_len, text_len,
