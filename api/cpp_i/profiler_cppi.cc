...
inagist_dashboard::Profile* g_profile;
...
int InitProfiler(const char* gist_maker_config) {
  ...
  g_profile = (inagist_dashboard::Profile*) NULL;
...
extern "C"
#endif
int CallGetProfileForHandle(const char* twitter_handle, unsigned int twitter_handle_len,
  ...
  *recommendations_count_ptr = 0;
  ...
  int ret_value = 0;
  if ((ret_value = g_profiler.GetProfile(twitter_handle, twitter_handle_len,
...
int CallCreateProfile() {
  if (g_profiler.CreateProfile(g_profile) < 0) {
    std::cerr << "ERROR: could not create profile" << std::endl;
    return -1;
  }
...
int CallAddTextToProfile(const char* text_buffer,
  ...
  if (g_profiler.AddTextToProfile((const unsigned char*) text_buffer, text_buffer_len, text_len, g_profile, dynamic_score) < 0) {
    std::cerr << "ERROR: could not delete profile" << std::endl;
    return -1;
  }
...
int CallGetProfile(
  ...
  *self_text_class_contributors_count_ptr = 0;
  ...
  int ret_value = 0;
  if (g_profiler.GetProfile(g_profile
#ifdef LANG_ENABLED
                         , self_languages_buffer, self_languages_buffer_len,
                         self_languages_len, self_languages_count
#endif // LANG_ENABLED
...
int CallDeleteProfile() {
  if (g_profiler.DeleteProfile(g_profile) < 0) {
    std::cerr << "ERROR: could not delete profile" << std::endl;
    return -1;
  }
  g_profile = (inagist_dashboard::Profile *) NULL;
