...
ERL_NIF_TERM nif_init_c(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
  ...
  memset(language_classifier_config_file_path, '\0', MAX_NAME_LEN);
  ...
  if (InitGistCollector(keytuples_config_file_path
#ifdef LANG_ENABLED
                    , language_classifier_config_file_path
#endif // LANG_ENABLED
#ifdef TEXT_CLASSIFICATION_ENABLED
...
ERL_NIF_INIT(gist_collector, nif_funcs, NULL, NULL, NULL, NULL)
