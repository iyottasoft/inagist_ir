...
# word,
# number of times word appears in file (which is hourly named_entities file)
...
def calculate_tf_idf_and_print(hash_freqs, num_distinct_words, num_words, named_entities_tweeters_map, outfile):
...
def main():
  ...
  file_name_pattern = 'named_entities_by_' + user_name + '_followers.*'
  ...
  for i in range(0, num_docs):
    if (i+1 > max_interval):
      break
    kfile = kfiles[i]
    split_word = 'named_entities_by_' + user_name + '_followers.'
    ...
    named_entities_tweeters_map_file_name = data_dir + '/' + user_name + '/' + 'named_entities_tweeters_map_for_' + user_name + '.' + time_stamp
    generate_map(named_entities_tweeters_map_file_name, named_entities_tweeters_map)
    ...
    if (interval_hash.has_key(i+1) or (i+1 == num_docs)):
      ...
      if (num_distinct_words_interval > 0 and num_words_interval > 0):
        ...
        calculate_tf_idf_and_print(cycle_hash_freqs, float(num_distinct_words_interval), float(num_words_interval), named_entities_tweeters_map, outfile)
        g_dashboard_fp.write('</td>\n')
        g_hash_tf_idf.clear()
      num_words_interval = 0
