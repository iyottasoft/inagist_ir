...
ERL_NIF_TERM nif_get_intent(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
  ...
  int ret_value = 0;
  if ((ret_value = GetIntent((unsigned char *) text_str, text_len,
                  (char *) safe_status, 10,
                  (char *) script, 4,
                  (char *) intent, MAX_BUFFER_LEN,
    ...
    return enif_make_atom(env, "error");
#else
    return enif_make_atom(env, "error_GetIntent");
#endif
  }
...
ERL_NIF_TERM nif_test_twitter_timeline(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
  ...
  while (tweet_start && tweet_end && *tweet_end != '\0') {
    ...
    *tweet_end = '\0';
    ...
    tuple6 = nif_get_intent(env, 1, arg_array);
...
{
  {"init_c", 1, nif_init_c},
  {"get_intent", 1, nif_get_intent},
  {"test_intent_twitter_timeline", 0, nif_test_twitter_timeline},
  {"test_intent_twitter_timeline", 1, nif_test_twitter_timeline},
  {"test_intent_file", 1, nif_test_from_file},
};
ERL_NIF_INIT(intent, nif_funcs, NULL, NULL, NULL, NULL)
