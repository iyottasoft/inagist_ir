...
GistCollector::GistCollector() {
#ifdef GC_DEBUG
  m_debug_level = GC_DEBUG;
#endif
}
...
int GistCollector::SetDebugLevel(unsigned int debug_level) {
  m_debug_level = debug_level;
  return 0;
}

int GistCollector::Init(const char* keytuples_extracter_config_file
#ifdef LANG_ENABLED
...
#ifdef GC_DEBUG
  if (m_debug_level > 2) {
    std::cout << "INFO: initializing language_classifier with config: " \
              << language_detection_config_file << std::endl;
  }
#endif // GC_DEBUG
  if (m_language_classifier.Init(language_detection_config_file) < 0) {
    std::cerr << "ERROR: could not initialize LanguageClassifier\n";
    return -1;
  }
#endif // LANG_ENABLED
...
int GistCollector::GetGist(const std::string& text) {
...
int GistCollector::GetGist(unsigned char* text_buffer,  const unsigned int text_buffer_len,
  ...
  *sentiment_buffer = '\0';
  ...
  if (strcmp(script_buffer, "en") != 0) {
    ...
    if (m_language_classifier.Classify(lang_class_words_buffer, lang_class_words_len, lang_class_words_count,
...
#ifdef TEXT_CLASSIFICATION_ENABLED
int GistCollector::FindTextClasses(inagist_classifiers::Corpus& corpus,
