...
int main(int argc, char *argv[]) {
  ...
  if (argc != 6) {
    std::cout << "Usage: " << argv[0] << " <stopwords file> <dictionary file> <unsafe dictionary file> <username> <named_entities output file>\n";
    return -1;
  }

#ifdef NAMED_ENTITIES_ENABLED
    std::cerr << "NAMED_ENTITIES_ENABLED. nothing to be done.\n";
    return -1;
  ...
  std::string named_entities_file = std::string(argv[5]);
...
#ifdef NAMED_ENTITIES_ENABLED
    ke.GetKeywords(buffer, safe_status, script, named_entities_set);
#endif // NAMED_ENTITIES_ENABLED
