#ifndef _INAGIST_CLASSIFIERS_LANGUAGE_CLASSIFIER_H_
#define _INAGIST_CLASSIFIERS_LANGUAGE_CLASSIFIER_H_
...
class LanguageClassifier : public Classifier {
 public:
  LanguageClassifier();
  ~LanguageClassifier();
  ...
  int GetCorpus(const std::string& text, std::string& script, Corpus& corpus);

  int InitTraining(const char *stopwords_file,
                   const char *dictionary_file,
                   const char *unsafe_dictionary_file);
  int ClassifyLanguage(std::set<std::string>& words_set,
                     std::string& guess_lang_output,
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(LanguageClassifier); 
};

} // inagist_classifiers

#endif // _INAGIST_CLASSIFIERS_LANGUAGE_CLASSIFIER_H_
