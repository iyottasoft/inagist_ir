...
int main(int argc, char* argv[]) {
  ...
  LanguageClassifier* lc = (LanguageClassifier*) new LanguageClassifier();
  if (lc->Init(corpus_config_file_name) < 0) {
    std::cout << "ERROR: could not initialize language classifier\n";
    return -1;
  }
  lc->SetDebugLevel(debug_level);
  ...
  if (0 == input_type) {
    while (getline(std::cin, line)) {
      ...
      if (lc->Classify(line, line.length(), lang,
