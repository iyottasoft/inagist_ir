...
int main(int argc, char* argv[]) {
  ...
  LanguageClassifier *ld = (LanguageClassifier*) new LanguageClassifier();

  bool ignore_history = true;
  if (ld->Init(classifier_config_file, ignore_history) < 0) {
    std::cerr << "ERROR: could not initialize language classifier\n";
    return -1;
  }
  ...
  if (ld->InitDependencies(my_argc, (char **) my_argv) < 0) {
    std::cerr << "ERROR: could not initialize dependencies for language classification\n";
    return -1;
  }
  free(my_argv[0]);
  ...
  try {
    ...
    if (ld->Clear() < 0) {
      std::cerr << "ERROR: could not clear language classifier\n";
      ret_val = -1;
    }
