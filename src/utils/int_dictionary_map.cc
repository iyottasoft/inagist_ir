...
//
int IntDictionaryMap::Load(const char* file) {
  if (NULL == file) {
  ...
  while (getline(ifs, line)) {
    ...
    sscanf(value.c_str(), "%d", &value_int);
    m_dictionary_map[key] = value_int;
  }
...
int IntDictionaryMap::Find(const unsigned char *key, int& value) {
  if (!key) {
...
int IntDictionaryMap::FindPart(const unsigned char* key, int& value) {
